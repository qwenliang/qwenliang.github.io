...
def cost(input,output):
    # Define the cost per token provided by the API 
    cost_per_input=0.00001 #for gpt4-turbo model
    cost_per_output=0.00003
    # Calculate the total cost
    input_cost = len(encoder.encode(input)) * cost_per_input
    output_cost= len(encoder.encode(output)) * cost_per_output
    tcost=round(input_cost + output_cost, 4)
    return tcost
...
def prompt_formatting(
    ...
    - deployment_name: Model to run generation with ('gpt-4-turbo', 'gpt-35-turbo', 'mistral-7b-instruct')
...
def generate_topics(
    ...
    for i, doc in enumerate(tqdm(docs)):
        ...
        try:
            ...
            single_cost=cost(prompt,response)
            ...
            if verbose:
                ...
                print(f"total_costs:{round(total_costs,4)}")
    ...
    return responses, topics_list, topics_root, total_costs
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-4-turbo",
        help="model to run topic generation with ('gpt-4-turbo', 'gpt-35-turbo', 'mistral-7b-instruct)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prompt_file",
        type=str,
        default="prompt/tax_generation_1.txt",
    ...
    parser.add_argument(
        "--out_file",
        type=str,
        default="data/output/tax_ouput_1.jsonl",
    ...
    if deployment_name == "gpt-4-turbo":
        context = 128000
    ...
    # Prompting ----
    time.sleep(20)
    responses, topics_list, topics_root, total_costs= generate_topics(
        topics_root,
        topics_list,
        context_len,
        docs,
