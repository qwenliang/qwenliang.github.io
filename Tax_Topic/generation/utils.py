...
# Add  API key to the environment variable & load it here.
PERPLEXITY_API_KEY = ""
...
def api_call(prompt, deployment_name, temperature, max_tokens, top_p):
    ...
    - deployment_name: name of the deployment to use (e.g. gpt4-32k, gpt-3.5-turbo, etc.)
    - temperature: temperature parameter
    - max_tokens: max tokens parameter
    - top_p: top p parameter
    """
    time.sleep(5)  # Change to avoid rate limit
    if deployment_name in ["gpt-35-turbo", "gpt-4-turbo", "gpt-3.5-turbo"]:        
...
def num_tokens_from_messages(messages, model):
    ...
    if model.startswith("gpt"):
        ...
        if model in ["gpt-35-turbo", "gpt-4-turbo", "gpt4-32k", "gpt-3.5-turbo"]:
...
def construct_document(docs, context_len):
    ...
    doc_str, doc_prompt = "", []
    while i < len(docs):
        if num_tokens_from_messages(docs[i], "gpt-4-turbo") < context_len // 5:
            to_add = f"Document {i+1}\n" + " ".join(docs[i].split("\n")) + "\n"
        else:
            to_add = (
                f"Document {i+1}\n"
                + truncating(docs[i], "gpt-4-turbo", context_len // 5)
                + "\n"
            )
            print(
                f"Truncating {num_tokens_from_messages(docs[i], 'gpt-4-turbo')} to {context_len//5}...."
            )
        if (num_tokens_from_messages(doc_str + to_add, "gpt-4-turbo")) >= context_len:
